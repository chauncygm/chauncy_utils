...
public class ClientExporter {
    ...
    public static final ClientExporter INSTANCE = new ClientExporter();

    private final Configuration cfg;

    private ClientExporter() {
        cfg = new Configuration(VERSION_2_3_34);
        cfg.setClassForTemplateLoading(ServerExporter.class, "/ftl/client");
    ...
    public void export(Map<Integer, SheetInfo> sheetMap, ExportOption option) {
        logger.info("MODE: {}, 开始导出代码和json配置文件... \n输出代码路径: {}\n输出json文件路径：{}",
                option.getMode(), option.getCSharpOutputPath().toFile().getAbsolutePath(),
                option.getCSharpJsonOutputPath().toFile().getAbsolutePath());
    ...
    public void writeClass(SheetInfo sheetInfo, ExportOption option) {
        File file = buildOutputPath(option, "Bean", "Cfg" + firstCapital(sheetInfo.getSheetName()) + ".cs").toFile();
        Map<String, Object> dataModel = Map.of("data", sheetInfo, "namespace", option.getCSharpNamespace());
        writeTemplate(file, "class.ftl", dataModel);
    }

    public void writeDefineClass(Collection<SheetInfo> sheetInfos, ExportOption option) {
        File file = buildOutputPath(option, "CfgDefine.cs").toFile();
        Map<String, Object> dataModel = Map.of("data", sheetInfos, "namespace", option.getCSharpNamespace());
    ...
    private Path buildOutputPath(ExportOption option, String... subPaths) {
        Path path = option.getCSharpOutputPath();
